...
export default function AnalyticsPage() {
  ...
  useEffect(() => {
    async function loadData() {
      ...
      try {
        ...
        // 2. FETCH: Sends the current page number to your server
  ...
  if (!data && loading) return <div className="flex h-screen items-center justify-center bg-white"><Loader2 className="animate-spin h-10 w-10 text-blue-500" /></div>
...
// --- UPDATED STAT CARD COMPONENT (Matches Dashboard Style) ---
function AnalyticsStatCard({ title, icon, items, fallback }: any) {
  // We extract the #1 top item to display as the "Big Hero Number"
    ...
    <Card className="border border-gray-100 shadow-none rounded-xl p-6 bg-white hover:border-blue-500 transition-all group cursor-default">
      <div className="flex items-center justify-between">
        <div className="space-y-1 max-w-[80%]">
          ...
          {/* Label (The name of the top country/device) */}
